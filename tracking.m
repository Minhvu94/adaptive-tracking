...
[Phi_real,Psi_p_real,JPhi_real] = compute_Phi_and_JPhi(p,F_real,[xi1 xi2 xi3 xi4 xi5],dt);
...
save('cart_pole_real.mat','Phi_real','Psi_p_real','JPhi_real')
...
% load('cart_pole_real.mat','Phi_real','Psi_p_real','JPhi_real')
...
for iter = 1:iter_max
        % Apply u to the real system 
        [~, x_trajJ_fine] = adaptive_taylor(p,Phi_real,Psi_p_real,[0 dt],[x;u]); 
        x = x_trajJ_fine(end,:)'; 
        x = x(1:4);
        x_traj = [x_traj, x]; 
        ...
        % Use the model to calculate A,B
        R_big = JPhi(dt,x(1),x(2),x(3),x(4),u_ref(iter+1));          
        A_store{iter} = R_big(1:4,1:4);     % A in Ax+Bu    
        B_store{iter} = R_big(1:4,5:5);     % B in Ax+Bu 
        
        [K,~,~] = dlqr(A_store{iter},B_store{iter},Q,R,[]); 

%         du = B_store{iter}\( A_store{iter}*(x-x_traj_ref(:,iter)) );
        dx = x-x_traj_ref(:,iter);
        du = -K*dx;

        u = u_ref(iter+1) + du;
        
        fprintf('iter = %.0f; dx = %.6f; du = %.4f;\n', [iter, norm(dx), du]) 
        store_error = [store_error norm(dx)];
